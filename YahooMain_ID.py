...
class LearnWStruct(WStruct_batch_Cons):
    def __init__(self, featureDimension, lambda_, userNum, W, RankoneInverse=False):
        WStruct_batch_Cons.__init__(self, featureDimension=featureDimension, lambda_=lambda_, userNum=userNum,
                                    windowSize=userNum, RankoneInverse=RankoneInverse)
        self.learn_stats = articleAccess()
...
class EgreedyContextualStruct(EgreedyContextualSharedStruct):
    def __init__(self, Tu, m, lambd, alpha, userNum, itemNum, k, feature_dim, tau=0.05, r=0, init='zero'):
        EgreedyContextualSharedStruct.__init__(self, Tu, m, lambd, alpha, userNum, itemNum, k, feature_dim, tau=0.05,
                                               r=0, init='zero')
        self.learn_stats = articleAccess()
...
if __name__ == '__main__':
    ...
    def printWrite():
        ...
        if algName == 'LearnW':
            ...
            recordedStats = [randomLearnCTR, LearnWCTR, LearnW_USERS.learn_stats.accesses,
                             LearnW_USERS.learn_stats.clicks]
        if algName == 'GOBLin':
        ...
        if algName == 'HybridLinUCB':
            ...
            recordedStats = [randomLearnCTR, HybridLinUCBCTR, HybridLinUCB_USERS.learn_stats.accesses,
                             HybridLinUCB_USERS.learn_stats.clicks]
        if algName == 'LinUCB':
        ...
        if algName == 'EgreedyContextual':
            ...
            recordedStats = [randomLearnCTR, EgreedyContextualCTR, EgreedyContextual.learn_stats.accesses,
                             HybridLinUCB_USERS.learn_stats.clicks]
    ...
    parser.add_argument('--alg', dest='alg',
                        help='Select a specific algorithm, could be CoLin, GOBLin, LinUCB, HybridLinUCB, LearnW, EgreedyContextual')
    ...
    EgreedyContextual = EgreedyContextualStruct(Tu=200, m=10, lambd=0.1, alpha=2000, userNum=userNum, itemNum=200000,
                                                k=2 + 5, feature_dim=5, init='zero')
            ...
            for line in f:
                ...
                for article in pool_articles:
                    ...
                    if len(article_featureVector) == 5:
                        ...
                        if algName == 'EgreedyContextual':
                            EgreedyContextual_pta = EgreedyContextual.getProb(article_id, currentUserID,
                ...
                if algName == 'EgreedyContextual':
                    if EgreedyContextualPicked == article_chosen:
                        ...
                        EgreedyContextual.updateParameters(click, EgreedyContextualPicked, currentUserID,
                                                           EgreedyContextual_PickedfeatureVector)
                        calculateStat()
