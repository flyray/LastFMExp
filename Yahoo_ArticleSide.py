...
class CoLinUCBStruct(AsyCoLinUCBUserSharedStruct):
    def __init__(self, featureDimension, lambda_, articleNum, W):
        AsyCoLinUCBUserSharedStruct.__init__(self, featureDimension=featureDimension, lambda_=lambda_,
                                             userNum=articleNum, W=W)
        self.learn_stats = articleAccess()
...
if __name__ == '__main__':
    ...
    def printWrite():
        ...
        if algName == 'CoLin':
            ...
            recordedStats = [randomLearnCTR, CoLinUCBCTR, CoLinUCB_ARTICLES.learn_stats.accesses,
                             CoLinUCB_ARTICLES.learn_stats.clicks]
        if algName == 'GOBLin':
            GOBLinCTR = GOBLin_ARTICLES.learn_stats.updateCTR()
            print totalObservations
            print 'random', randomLearnCTR, '  GOBLin', GOBLinCTR
            recordedStats = [randomLearnCTR, GOBLinCTR, GOBLin_ARTICLES.learn_stats.accesses,
                             GOBLin_ARTICLES.learn_stats.clicks]
        if algName == 'Uniform_LinUCB':
            ...
            recordedStats = [randomLearnCTR, UniformLinUCBCTR, LinUCB_ARTICLES.learn_stats.accesses,
                             LinUCB_ARTICLES.learn_stats.clicks]
        if algName == 'LinUCB':
    ...
    parser.add_argument('--ArticleDictFile', dest="ArticleDictFilename",
                        help="input the file for Article FeatureVectors")
    ...
    parser.add_argument('--Sparsity', dest='SparsityLevel',
                        help='Set the SparsityLevel by choosing the top M most connected ARTICLES, should be smaller than userNum, when equal to userNum, we are using a full connected graph')
