...
# pre-process events
file = open(sys.argv[1] + 'user_taggedartists-timestamps.dat', 'r')
file.readline()
raw_user_artist_tag = []
...
vocab = {}
tag2word = {}
file = open(sys.argv[1] + 'tags.dat', 'r')
file.readline()
for line in file:
    arr = line.strip().split('\t')
...
# print to file
fvectors = open(sys.argv[1] + 'feature_vectors.dat', 'w')
scipy.io.mmwrite(fvectors, result)  # To read the matrix back, use scipy.io.mmread()
fvectors.close()
fartistidx = open(sys.argv[1] + 'arm_idx.dat', 'w')
