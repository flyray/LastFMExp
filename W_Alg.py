            ...
            for i in range(len(self.W)):
                if len(self.W_X_arr[i]) != 0:
                    ...
                    def fun_WRegu(w):
                        w = np.asarray(w)
                        return np.sum((np.dot(self.W_X_arr[i], w) - self.W_y_arr[i]) ** 2,
                                      axis=0) + self.lambda_ * np.linalg.norm(w - self.W.T[i]) ** 2
                    ...
                    current = self.W.T[i]
                    if self.L1Regu:
                        res = minimize(fun_l1, current, constraints=getcons(len(self.W)), method='SLSQP',
                                       bounds=getbounds(len(self.W)), options={'disp': False})
                    elif self.WRegu:
                        res = minimize(fun_WRegu, current, constraints=getcons(len(self.W)), method='SLSQP',
                                       jac=evaluateGradient_WRegu, bounds=getbounds(len(self.W)),
                                       options={'disp': False})
                    else:
                        res = minimize(fun, current, constraints=getcons(len(self.W)), method='SLSQP',
                                       jac=evaluateGradient, bounds=getbounds(len(self.W)), options={'disp': False})
                    self.W.T[i] = res.x
...
class LearnWAlgorithm:
    def __init__(self, dimension, alpha, lambda_, eta_, n, windowSize, RankoneInverse=False,
                 WRegu=False):  # n is number of users
