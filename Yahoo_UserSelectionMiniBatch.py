...
if __name__ == '__main__':
    ...
    def printWrite():
        ...
        if algName == 'CoLin':
            ...
            recordedStats = [randomLearnCTR, CoLinUCBCTR, CoLinUCB_USERS.learn_stats.accesses,
                             CoLinUCB_USERS.learn_stats.clicks]
        if algName == 'GOBLin':
            GOBLinCTR = GOBLin_USERS.learn_stats.updateCTR()
            print totalObservations
            print 'random', randomLearnCTR, '  GOBLin', GOBLinCTR
            recordedStats = [randomLearnCTR, GOBLinCTR, GOBLin_USERS.learn_stats.accesses,
                             GOBLin_USERS.learn_stats.clicks]
        if algName == 'Uniform_LinUCB':
            ...
            recordedStats = [randomLearnCTR, UniformLinUCBCTR, LinUCB_USERS.learn_stats.accesses,
                             LinUCB_USERS.learn_stats.clicks]
        if algName == 'LinUCB':
    ...
    parser.add_argument('--alg', dest='alg',
                        help='Select a specific algorithm, could be CoLin, GOBLin, LinUCB, or Uniform_LinUCB')
    ...
    userSelectionMiniBatch = 20
    timeRun = datetime.datetime.now().strftime('_%m_%d_%H_%M')  # the current data time
    dataDays = ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10']
    fileSig = 'UserSelection_' + str(DiagType) + str(clusterNum) + 'SP' + str(SparsityLevel) + algName + 'batch_' + str(
        userSelectionMiniBatch) + '_'
    batchSize = 2000
    ...
    for dataDay in dataDays:
        ...
        fileName = yahooData_address + "/ydata-fp-td-clicks-v1_0.200905" + dataDay + '.' + str(clusterNum) + '.userID'
        ...
        with open(fileName, 'r') as f:
            ...
            for line in f:
                ...
                totalObservations += 1
                userSelectionMiniBatch_Counter += 1
                if userSelectionMiniBatch_Counter <= userSelectionMiniBatch:
                    MiniBatch_Lines.append(line)
                    if len(MiniBatch_Lines) == userSelectionMiniBatch:
                        # print MiniBatch_Lines
                        CoLinUCB_chosenLine = []
                        GOBLin_ChosenLine = []
                        UniformLinUCB_ChosenLine = []
                        LinUCB_chosenLine = []
                        for i in range(len(MiniBatch_Lines)):
                            currentArticles = []
                            ...
                            for batchLine in MiniBatch_Lines:
                                tim, article_chosen, click, currentUserID, pool_articles = parseLine_ID(batchLine)
                                if algName == 'CoLin' and batchLine not in CoLinUCB_chosenLine:
                                    for article in pool_articles:
                                        ...
                                        if len(article_featureVector) == 5:
                                            CoLinUCB_pta = CoLinUCB_USERS.getProb(alpha, article_featureVector,
                                                                                  currentUserID)
                                            if CoLinUCB_maxPTA < CoLinUCB_pta:
                                                ...
                                                CoLinUCBPickedLine = batchLine
                                if algName == 'GOBLin' and batchLine not in GOBLin_ChosenLine:
                                    for article in pool_articles:
                                        article_id = int(article[0])
                                        article_featureVector = np.asarray(article[1:6])
                                        currentArticles.append(article_id)
                                        if len(article_featureVector) == 5:
                                            GOBLin_pta = GOBLin_USERS.getProb(alpha, article_featureVector,
                                                                              currentUserID)
                                            if GOBLin_maxPTA < GOBLin_pta:
                                                ...
                                                GOBLinPickedline = batchLine

                                if algName == 'Uniform_LinUCB' and batchLine not in UniformLinUCB_ChosenLine:
                                    for article in pool_articles:
                                        article_id = int(article[0])
                                        article_featureVector = np.asarray(article[1:6])
                                        currentArticles.append(article_id)
                                        if len(article_featureVector) == 5:
                                            ...
                                            if UniformLinUCB_maxPTA < UniformLinUCB_pta:
                                                ...
                                                UniformLinUCBPickedline = batchLine

                                if algName == 'LinUCB' and batchLine not in LinUCB_chosenLine:
                                    for article in pool_articles:
                                        article_id = int(article[0])
                                        article_featureVector = np.asarray(article[1:6])
                                        currentArticles.append(article_id)
                                        if len(article_featureVector) == 5:
                                            LinUCB_pta = LinUCB_users[currentUserID].getProb(alpha,
                                            ...
                                            if LinUCB_maxPTA < LinUCB_pta:
                                                ...
                                                LinUCBPickedline = batchLine

                                            # print article_id, LinUCB_pta
                                            # time.sleep(1)
                                            # print 'Yes'
                            if algName == 'CoLin':
                                CoLinUCB_chosenLine.append(CoLinUCBPickedLine)
                                ...
                                # calculateStat()
                            if algName == 'GOBLin':
                                GOBLin_ChosenLine.append(GOBLinPickedline)
                                tim, article_chosen, click, currentUserID, pool_articles = parseLine_ID(
                                    GOBLinPickedline)
                                ...
                                # calculateStat()
                            if algName == 'Uniform_LinUCB':
                                UniformLinUCB_ChosenLine.append(UniformLinUCBPickedline)
                                tim, article_chosen, click, currentUserID, pool_articles = parseLine_ID(
                                    UniformLinUCBPickedline)
